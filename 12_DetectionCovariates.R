...
# 
# I have all the detection histories made (but I'll need to double check with the
#    the actual data that it still works), and now I need to add in the survey 
...
# same workflow as temperature
     ...
     date_m <- matrix(date_vec, ncol = 18)
     rownames(date_m) <- l_p
...
# same workflow as temperature
     ...
     cloud_char <- as.character(cloud_vec)
     cloud_m <- matrix(cloud_char, ncol = 18)
     rownames(cloud_m) <- l_p
     cloud_y_v <- paste0("Cloud_",y_v)
     colnames(cloud_m) <- cloud_y_v
# clean up
rm(cloud,cloud_unique,cloud_sort,cloud_vec,cloud_y_v, cloud_char)
...
# same workflow as temperature
     ...
     wind_char <- as.character(wind_vec)
     wind_m <- matrix(wind_char, ncol = 18)
     rownames(wind_m) <- l_p
     wind_y_v <- paste0("Wind_",y_v)
     colnames(wind_m) <- wind_y_v
# clean up
rm(wind,wind_unique,wind_sort,wind_vec,wind_y_v, wind_char)
...
# same workflow as temperature
     ...
     sky_char <- as.character(sky_vec)
     sky_m <- matrix(sky_char, ncol = 18)
     rownames(sky_m) <- l_p
     sky_y_v <- paste0("Sky_",y_v)
     colnames(sky_m) <- sky_y_v
# clean up
rm(sky,sky_unique,sky_sort,sky_vec,sky_y_v, sky_char)
